...
exports.sumTokens = exports.DENOMS_IN_CENTS = void 0;
exports.mint = mint;
exports.getExact = getExact;
exports.containsExactly = containsExactly;
exports.pick = pick;
exports.without = without;
const sum = (arr) => arr.reduce((acc, cur) => (acc += cur.value_in_cents), 0);
exports.sumTokens = sum;
const desc = (arr) => arr.sort((a, b) => b.value_in_cents - a.value_in_cents);
exports.DENOMS_IN_CENTS = [
    1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 50000,
].reverse();
/**Greedily mints new currency, creating as few new tokens as possible.*/
function mint(amountInCents, maxDenom = 50000) {
    let mintedBills = [];
    while (amountInCents > 0) {
        const denom = exports.DENOMS_IN_CENTS.find(den => den < amountInCents || den === amountInCents);
        amountInCents -= denom;
        mintedBills.push({ value_in_cents: denom });
    }
    return desc(mintedBills);
}
function getExact(bills, targetAmtInCents) {
    const da = [];
    let total = 0;
    const selected = [];
...
function containsExactly(bills, amt) {
...
function getClosest(bills, targetAmtInCents, aboveNegative = false) {
    const da1 = [];
...
function pick(bills, targetAmtInCents) {
...
function without(bills, amountInCents) {
    const temp = [...bills];
