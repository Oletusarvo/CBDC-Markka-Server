...
export const getTransactions = createHandler(async (req: AuthenticatedExpressRequest, res) => {
  ...
  const acc = await db('account').where({ user_id: session.user.id }).select('id').first();
  const transactions = await db('transaction')
    .where({ from: acc.id })
    .orWhere({ to: acc.id })
    .leftJoin(
      db.select('id', 'user_id').from('account').as('from_acc').groupBy('id'),
      'from_acc.id',
      'transaction.from',
    )
    .leftJoin(
      db.select('user_id', 'id').from('account').as('to_acc').groupBy('id'),
      'to_acc.id',
      'transaction.to',
    )
    .leftJoin(
      db.select('email', 'id').from('user').as('from_user').groupBy('id'),
      'from_user.id',
      'from_acc.user_id',
    )
    .leftJoin(
      db.select('email', 'id').from('user').as('to_user').groupBy('id'),
      'to_user.id',
      'to_acc.user_id',
    )
